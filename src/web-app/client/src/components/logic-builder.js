...
export default class LogicBuilder extends Component {
  ...
  componentDidMount(){
    axios.get("http://localhost:4000/api/logic")
      .then(res=>{
        this.setState({tempMin: res.data.tempMin,
  ...
  onSubmit=(e)=>{
    ...
    ||(this.state.humidityMax<0||
      ...
      axios.post("http://localhost:4000/api/logic",logic)
        .then(res=>{
  ...
  errorprompt=()=>{
    return(
      <div style={{color:'red'}}>Enter all fields with valid numbers only</div>
    )
  }
  successprompt=()=>{
    return(
      <div style={{color:'green', background:'lightgreen', padding:5}}>  Logic has been changed</div>
    )
  }
  render() {
    return (
      <ModalTemplate active={true} title="Logic Builder" reset={this.props.reset} onSubmit={this.onSubmit}>
        Minimum Temperature: <input type="Number" class="form-control" value={this.state.tempMin} onChange={this.onChangeTempMin}/><br/>
        Maximum Temperature: <input type="Number" class="form-control" value={this.state.tempMax} onChange={this.onChangeTempMax}/><br/>
        Minimum Humidity: <input type="Number" class="form-control" value={this.state.humidityMin} min='0' max ='100'onChange={this.onChangeHumidityMin}/><br/>
